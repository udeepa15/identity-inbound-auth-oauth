    ...
    public CarbonOAuthTokenRequest(HttpServletRequest request) throws OAuthSystemException,
        ...
        if (request.getParameterNames() != null) {
            ...
            while (request.getParameterNames().hasMoreElements()) {
                ...
                // Use getParameterValues to support multiple values for the same parameter (e.g., audience)
                String[] values = request.getParameterValues(key);
                requestParameterList.add(new RequestParameter(key, values));
            }
            requestParameters =
        ...
        if (OAuthUtils.isEmpty(requestTypeValue)) {
            if (LoggerUtils.isDiagnosticLogsEnabled()) {
                LoggerUtils.triggerDiagnosticLogEvent(new DiagnosticLog.DiagnosticLogBuilder(
                        ...
                        .resultMessage("Missing grant_type parameter value.")
        ...
        if (clazz == null) {
            ...
            if (LoggerUtils.isDiagnosticLogsEnabled()) {
                LoggerUtils.triggerDiagnosticLogEvent(new DiagnosticLog.DiagnosticLogBuilder(
                        ...
                        OAuthConstants.LogConstants.ActionIDs.VALIDATE_INPUT_PARAMS)
                        .inputParam(LogConstants.InputKeys.CLIENT_ID, getClientId())
                        .inputParam(OAuthConstants.LogConstants.InputKeys.GRANT_TYPE, requestTypeValue)
                        .configParam(OAuthConstants.LogConstants.ConfigKeys.SUPPORTED_GRANT_TYPES,
                                OAuthServerConfiguration.getInstance().getSupportedGrantTypeValidators().keySet())
                        .resultMessage("Unsupported grant_type value.")
                        .logDetailLevel(DiagnosticLog.LogDetailLevel.APPLICATION)
                        .resultStatus(DiagnosticLog.ResultStatus.FAILED));
            }
